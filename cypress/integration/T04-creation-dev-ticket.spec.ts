describe('creation festival', () => {
  ...
  it('fill title', () => {
    cy.get('tui-input').eq(0).type('creation festival dev par cypress');
  });
  ...
  it('fill software', () => {
    cy.get('tui-select[formControlName="project"]').click();
    cy.get('tui-data-listFestival.ng-star-inserted > :nth-child(2)').click();
  });

  it('fill title', () => {
    cy.get('quill-editor').type('description festival dev depuis cypress');
  });
